...
@pytest.mark.integration
def test_complete_marks_dequeued_task_done_against_real_postgres(
    postgres_pool: db.ConnectionPool,
) -> None:
    with postgres_pool.connection() as conn:
    ...
    task = db.dequeue("ticketflow-agent", "worker-1", pool=postgres_pool)
...
@pytest.mark.integration
def test_fail_backoff_schedule_is_observable_against_real_postgres(
    postgres_pool: db.ConnectionPool,
) -> None:
    with postgres_pool.connection() as conn:
    ...
    first = db.dequeue("ticketflow-agent", "worker-1", pool=postgres_pool)
    ...
    second = db.dequeue("ticketflow-agent", "worker-1", pool=postgres_pool)
...
@pytest.mark.integration
def test_fail_gives_up_when_attempts_exhausted_against_real_postgres(
    postgres_pool: db.ConnectionPool,
) -> None:
    with postgres_pool.connection() as conn:
...
@pytest.mark.integration
def test_fail_permanent_task_goes_straight_to_failed_against_real_postgres(
    postgres_pool: db.ConnectionPool,
) -> None:
    with postgres_pool.connection() as conn:
        task_id = _enqueue(conn, max_attempts=5)
        conn.execute("UPDATE task_queue SET permanent = true WHERE id = %s", (task_id,))
        conn.commit()
...
@pytest.mark.integration
def test_complete_and_fail_ignore_non_leased_rows_against_real_postgres(
    postgres_pool: db.ConnectionPool,
) -> None:
    with postgres_pool.connection() as conn:
        _enqueue(conn)
...
@pytest.mark.integration
def test_reclaim_expired_makes_dropped_lease_redeliverable(
    postgres_pool: db.ConnectionPool,
) -> None:
    with postgres_pool.connection() as conn:
        task_id = _enqueue(conn)
    ...
    leased = db.dequeue("ticketflow-agent", "worker-1", pool=postgres_pool)
    ...
    redelivered = db.dequeue("ticketflow-agent", "worker-2", pool=postgres_pool)
...
@pytest.mark.integration
def test_reclaim_expired_leaves_live_leases_alone(
    postgres_pool: db.ConnectionPool,
) -> None:
    with postgres_pool.connection() as conn:
        task_id = _enqueue(conn)
...
@pytest.mark.integration
def test_concurrent_dequeue_delivers_each_task_once(
    postgres_pool: db.ConnectionPool,
) -> None:
    task_count = 50
    worker_count = 8
    ...
    def drain(worker_id: str) -> list[int]:
        ...
        ids: list[int] = []
        while True:
            task = db.dequeue("ticketflow-agent", worker_id, pool=postgres_pool)
            if task is None:
                break
            ids.append(task.id)
        return ids
